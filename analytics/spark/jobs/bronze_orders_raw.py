...
TOPIC     = os.getenv("TOPIC_ORDER_EVENTS", "orders.events")
LAKE      = os.getenv("LAKEHOUSE_URI", "s3a://delta-lake")

OUT_PATH  = f"{LAKE}/bronze_raw/orders"
CKPT_PATH = f"{LAKE}/checkpoints/bronze_raw/orders"

def main():
    spark = (SparkSession.builder
            .appName("bronze_raw_orders")
    ...
    spark.sql(f"""
      CREATE TABLE IF NOT EXISTS bronze_raw.orders_raw
      USING DELTA
      LOCATION '{OUT_PATH}'
    """)
    ...
    q = (raw.writeStream
        ...
        .toTable("bronze_raw.orders_raw"))  # Changed from .start() to .toTable()
